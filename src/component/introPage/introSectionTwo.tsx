import './intoSection2.css'
import { useState } from 'react';

export default function IntroSectionTwo(){
    let [brown_Ani, setbrown_Ani] = useState<boolean>(false);
    
    return(
        <div className="section">
                <div className="grayBox2">
                <p>
                웹 개발자를 목표로 공부를 하고 있습니다.
                <br></br>
                Node.js 환경에 익숙합니다.
                <br></br>
                매일 공부하고 기록합니다.
                <br></br>
                <br></br>
                
                적응력, 소통 능력,
                <br></br>
                 집중력, 참을성,
                 <br></br>
                 성취욕이 장점입니다.
                 <br></br>
                 <br></br>
                 아래 버튼을 눌러 저의 이력을 살펴보세요.
                </p>
                <button className="cvBtn">Download CV</button>
            </div>
            <div className={`brownBox2 ${brown_Ani&&'grayBox2-ani'}`}>

                <div className='myPrice'>My price</div>
                <div className='chartSection'>
                    <span className='chart'>now</span>
                    <span className='chart'></span>
                    <span className='chart'>future</span>
                </div>
                <p className='infoP'>
                    contact me
                    <br></br>
                    email: wjdwl545@naver.com
                    <br></br>
                    phon: 010-4346-4228
                </p>
            </div>
        </div>
    )
}